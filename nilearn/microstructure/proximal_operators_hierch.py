...
import numpy as np
from ..decoding.objective_functions import _tv_l1_from_gradient, _div_id, _gradient_id, _unmask
...
def _prox_tvl1_with_intercept_hierch(w, shape, l1_ratio, weight, dgap_tol, 
                                     mask, max_iter=5000, init=None,
                                     verbose=False):
    """
    Computation of TV-L1 prox, taking into account the intercept.
    ...
    init : ndarray, shape (w_size - 1,), optional (default None)
        Initialization vector for the prox. FOR ONE MEASURE ONLY
    ...
    for j in range(measures):
        ...
        w_j = _unmask(w_j, mask)
        init_j = _unmask(init_j, mask)

        init_j = init_j.reshape(shape) if not init_j is None else init_j

        out_loc, prox_info_loc = _prox_tvl1(
        ...
        if j == 0:
            ...
            out = np.append(out, out_loc)

        prox_info_all.append(prox_info_loc)

    info = next((item for item in prox_info_all if item["converged"] == False),
                False)

    if info:
        prox_info = dict(converged=False)
    else:
